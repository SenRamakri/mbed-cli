...
# crash-decode command
@subcommand('crash-decode',
    dict(name=['-d', '--crashdump'], help='Path to file with crash dump contents'),
    dict(name=['-e', '--elffile'], help='Path to ELF file for the build which generated the crash'),
    dict(name=['-m', '--mapfile'], help='Path to MAP file for the build which generated the crash'),
    hidden_aliases=['crd'],
    help='Decodes crash dump from Mbed-os fault handler\n\n',
    description=(
        "Decodes crash dump generated by Mbed-OS fault handlers. When the system\n"
        "encounters a fault exception, Mbed-OS fault handler is invoked and it\n"
        "generates a crash dump with system context which includes register dump\n"
        "and current thread information. This command helps decoding the crash\n"
        "dump to generate more detailed information."))
        
def crash_decode(crashdump=None, elffile=None, mapfile=None):
    
    info(("\ncrashdump=%s elffile=%s mapfile=%s\n"%(crashdump, elffile, mapfile)))
    
    if(crashdump is None):
        error("No crash dump file specified. Please see \"mbed crash-decode --help\" for details.")
    ...
    info(("popen cmdline: %s"%([python_cmd, '-u', os.path.join('/', 'crash_decode.py')]
                  + [crashdump]
                  + ([elffile] if elffile else [])
                  + ([mapfile] if mapfile else [])
                  )))
    ...
    if tools_dir:
        ...
        try:
            popen([python_cmd, '-u', os.path.join(tools_dir, 'crash_decode.py')]
                  + [crashdump]
                  + ([elffile] if elffile else [])
                  + ([mapfile] if mapfile else []),
                  env=env)
        except ProcessException as e:
            error("Failed to decode crash dump (run with '-vv' for more information)")
            if very_verbose:
                error(str(e))
    else:
        error("The mbed-os tools were not found in \"%s\" to decode the crash dump. Crash decode failed." % program.path)
